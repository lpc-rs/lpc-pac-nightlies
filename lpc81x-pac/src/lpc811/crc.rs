...
impl RegisterBlock {
    ...
    #[inline(always)]
    pub fn sum_wr_data_wr_data(&self) -> &SUM_WR_DATA_WR_DATA {
        unsafe {
            &*(((self as *const Self) as *const u8).add(8usize) as *const SUM_WR_DATA_WR_DATA)
        }
    }
    #[doc = "0x08 - CRC checksum register"]
    #[inline(always)]
    pub fn sum_wr_data_sum(&self) -> &SUM_WR_DATA_SUM {
        unsafe { &*(((self as *const Self) as *const u8).add(8usize) as *const SUM_WR_DATA_SUM) }
    }
}
