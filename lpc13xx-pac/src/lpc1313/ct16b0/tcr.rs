...
#[doc = "Field `CRESET` reader - When one, the Timer Counter and the Prescale Counter are synchronously reset on the next positive edge of PCLK. The counters remain reset until TCR\\[1\\]
is returned to zero."]
pub type CRESET_R = crate::BitReader<bool>;
#[doc = "Field `CRESET` writer - When one, the Timer Counter and the Prescale Counter are synchronously reset on the next positive edge of PCLK. The counters remain reset until TCR\\[1\\]
is returned to zero."]
pub type CRESET_W<'a, const O: u8> = crate::BitWriter<'a, u32, TCR_SPEC, bool, O>;
impl R {
...
is returned to zero."]
    #[inline(always)]
    pub fn creset(&mut self) -> CRESET_W<1> {
        CRESET_W::new(self)
    }
