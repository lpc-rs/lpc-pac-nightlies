...
pub type FRF_W<'a, const O: u8> = crate::FieldWriterSafe<'a, u32, CR0_SPEC, u8, FRF_A, 2, O>;
...
pub type CPOL_W<'a, const O: u8> = crate::BitWriter<'a, u32, CR0_SPEC, CPOL_A, O>;
...
impl CPHA_R {
    ...
    pub fn is_secondclok(&self) -> bool {
        *self == CPHA_A::SECONDCLOK
    }
}
#[doc = "Field `CPHA` writer - Clock Out Phase. This bit is only used in SPI mode."]
pub type CPHA_W<'a, const O: u8> = crate::BitWriter<'a, u32, CR0_SPEC, CPHA_A, O>;
impl<'a, const O: u8> CPHA_W<'a, O> {
    ...
    pub fn secondclok(self) -> &'a mut W {
        self.variant(CPHA_A::SECONDCLOK)
    }
}
...
impl W {
    ...
    #[doc = "Bit 6 - Clock Out Polarity. This bit is only used in SPI mode."]
    #[inline(always)]
    pub fn cpol(&mut self) -> CPOL_W<6> {
        CPOL_W::new(self)
    }
    #[doc = "Bit 7 - Clock Out Phase. This bit is only used in SPI mode."]
    #[inline(always)]
    pub fn cpha(&mut self) -> CPHA_W<7> {
        CPHA_W::new(self)
    }
    #[doc = "Bits 8:15 - Serial Clock Rate. The number of prescaler-output clocks per bit on the bus, minus one. Given that CPSDVSR is the prescale divider, and the APB clock PCLK clocks the prescaler, the bit frequency is PCLK / (CPSDVSR x \\[SCR+1\\])."]
    #[inline(always)]
